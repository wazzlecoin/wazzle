...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs)) {
        ...
        std::string strUsage = PACKAGE_NAME " wazzle-tx utility version " + FormatFullVersion() + "\n\n" +
            "Usage:  wazzle-tx [options] <hex-tx> [commands]  Update hex-encoded wazzle transaction\n" +
            "or:     wazzle-tx [options] -create [commands]   Create hex-encoded wazzle transaction\n" +
            "\n";
...
static int CommandLineRawTx(int argc, char* argv[])
{
    ...
    try {
        ...
        if (!fCreateBlank) {
            ...
            // param: hex-encoded wazzle transaction

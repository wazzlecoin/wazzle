...
const char * const WAZZLE_CONF_FILENAME = "wazzle.conf";
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager &am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "wazzled -foo=bar
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //wazzle-tx using stdin
        std::string val;
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Wazzle
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Wazzle
    // Mac: ~/Library/Application Support/Wazzle
    // Unix: ~/.wazzle
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "WAZ";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/WAZ";
#else
    // Unix
    return pathRet / ".WAZ";
#endif
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    const std::string confPath = GetArg("-conf", WAZZLE_CONF_FILENAME);
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    // Make sure Wazzle Core copyright is not removed by accident
    if (copyright_devs.find("Wazzle Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + "The Wazzle Core developers";
    }

Wazzle Core version 0.11.0 is now available from:

  <https://wazzle.org/bin/wazzle-core-0.11.0/>
...
For example, add the following to `wazzle.conf`:
...
related to the blockchain in the wazzle system: the raw blocks as received over 
the network (blk???.dat), the undo data (rev???.dat), the block index and the 
UTXO set (both LevelDB databases). The databases are built from the raw data.

Block pruning allows Wazzle Core to delete the raw block and undo data once 
...
block index and UTXO databases. The minimum was chosen so that Wazzle Core will 
...
For an example script see [wazzle-submittx](https://github.com/laanwj/wazzle-submittx).
...
- #5334 `cf87536` libwazzleconsensus: Add pkg-config support
...
- #6269 `95aca44` gitian: Use the new wazzle-detached-sigs git repo for OSX signatures
...
- #5729 `9c4a5a5` Allow unit changes for read-only WazzleAmountField
- #5753 `0f44672` Add wazzle logo to about screen
...
- #5809 `46bfbe7` Add wazzle-cli man page
...
- #5849 `41259ca` contrib/init/wazzled.openrc: Compatibility with previous OpenRC init script variables
...
- #6116 `30dc3c1` [OSX] rename Wazzle-Qt.app to Wazzle-Core.app
- #6168 `b3024f0` contrib/linearize: Support linearization of testnet blocks
- #6098 `7708fcd` Update Windows resource files (and add one for wazzle-tx)
